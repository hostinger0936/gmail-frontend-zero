...
export default function GmailNotificationsPage() {
  ...
  async function loadNotifications() {
    setLoading(true);
    try {
      const list = await listGmailNotifications(did);
      if (!mountedRef.current) return;

      const normalized = (Array.isArray(list) ? list : [])
        .filter((item) => isGmailNotification(item))
        .slice()
        .sort((a, b) => getTimestamp(b) - getTimestamp(a));

      setItems(normalized);
    } catch (e) {
      console.error("load gmail notifications failed", e);
      if (!mountedRef.current) return;
      setItems([]);
    } finally {
  ...
  useEffect(() => {
    ...
    const offMessage = wsService.onMessage((msg) => {
      ...
      if (type === "event" && event === "app-notification") {
        ...
        if (!isGmailNotification(data)) return;
  ...
  async function handleDelete(item: AppNotificationDoc) {
    ...
    if (!confirm("Delete this Gmail notification?")) return;
    ...
    try {
      ...
      console.error("delete gmail notification failed", e);
  ...
  async function handleDeleteAll() {
    if (!items.length) {
      alert("No Gmail notifications found");
      return;
    }

    if (!confirm("Delete all Gmail notifications for this device?")) return;
    ...
    try {
      ...
      setItems([]);
      alert("All Gmail notifications deleted");
    } catch (e) {
      console.error("delete all gmail notifications failed", e);
        ...
        <SurfaceCard className="p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-red-200 bg-red-50 text-2xl text-red-700">
                  📧
                </div>
                <div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            ...
            <button
              ...
              className="h-11 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 text-[13px] font-extrabold text-emerald-700 hover:bg-emerald-100"
              type="button"
            >
              Open WhatsApp
            </button>
            ...
            {loading ? (
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6 text-center text-slate-500">
                <div className="flex min-h-[220px] flex-col items-center justify-center gap-4">
                  <div className="font-extrabold text-slate-800">No Gmail notifications found</div>
                  <div className="max-w-[260px] text-[12px] text-slate-500">
                    Live Gmail notifications will appear here for this device.
                  </div>
                </div>
              </div>
            ) : (
